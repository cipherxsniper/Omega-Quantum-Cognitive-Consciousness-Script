...
# ------------------------------
# DYNAMICALLY IMPORT ALL OMEGA SCRIPTS
# ------------------------------

for file in glob.glob("omega_president*.py") + glob.glob("*.py"):
    try:
        spec = importlib.util.spec_from_file_location(file[:-3], file)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        print(f"[Omega Loader] Imported {file}")
    except Exception as e:
        print(f"[Omega Loader Error] Could not import {file}: {str(e)}")
...
def check_keywords(input_text):
    for keyword, action in keyword_actions.items():
        if keyword.lower() in input_text.lower():
            ...
            # Trigger backend self-upgrade automatically
            self_upgrade()
    ...
    try:
        ...
        for script_file in Path(os.getcwd()).glob("*.py"):
            with open(script_file, "r") as f:
                code = f.read()
            # Append upgrade log only if not already appended
            if "# Auto-upgraded at" not in code:
